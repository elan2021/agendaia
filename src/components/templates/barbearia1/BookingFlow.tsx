...
import { ChevronLeft, Check, Clock, Calendar, Send, Loader2 } from 'lucide-react';
...
const BookingFlow: React.FC<BookingFlowProps> = ({ onBack, onSuccess, services, professionals, tenantId }) => {
  ...
  useEffect(() => {
    if (!selectedBarber || !selectedService || !selectedDate || !tenantId) return;
    ...
    getPublicAvailableSlots(tenantId, selectedBarber.id, selectedService.id, dateStr)
      .then(res => setAvailableSlots(res.slots || []))
      .finally(() => setLoadingSlots(false));
  }, [selectedBarber, selectedService, selectedDate, tenantId]);
            ...
            <div className={`${styles.stepBlock} ${styles.stepBlockRevealed}`}>
              ...
              {loadingSlots ? (
                <div style={{ display: 'flex', justifyContent: 'center', padding: '24px' }}>
                  <Loader2 size={24} className="animate-spin" style={{ color: '#2d2d2d' }} />
                </div>
