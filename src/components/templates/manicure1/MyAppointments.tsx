...
import { ChevronLeft, Calendar as CalendarIcon, Clock, User, Trash2, MessageCircle } from 'lucide-react';
...
const MyAppointments: React.FC<MyAppointmentsProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('manicure1_appointments');
    if (saved) {
  ...
  const handleCancel = (index: number) => {
    if (confirm('Deseja realmente cancelar este agendamento?')) {
      ...
      localStorage.setItem('manicure1_appointments', JSON.stringify(updated));
    }
  };
