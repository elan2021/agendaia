      ...
      <div className={styles.apptsBody}>
        {step === 'phone' ? (
          <form onSubmit={handleSearch} style={{ display: 'flex', flexDirection: 'column', gap: '16px', padding: '20px' }}>
             <p style={{ color: '#666', fontSize: '14px', textAlign: 'center', marginBottom: '10px' }}>
          ...
          appointments.length === 0 ? (
            <div className={styles.apptEmpty}>
              ...
              <button onClick={() => setStep('phone')} style={{ marginTop: '20px', color: '#ff6b6b', textDecoration: 'underline', background: 'transparent', border: 'none', cursor: 'pointer' }}>Nova Busca</button>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <button onClick={() => setStep('phone')} style={{ alignSelf: 'center', color: '#ff6b6b', textDecoration: 'underline', background: 'transparent', border: 'none', cursor: 'pointer', marginBottom: '10px' }}>Buscar outro número</button>
                ...
                <div key={idx} className={styles.apptCard}>
                  ...
                  <div className={styles.apptActions}>
                    ...
                    <button className={`${styles.apptBtn} ${styles.apptBtnWhats}`}>
                      <MessageCircle size={14} style={{ marginRight: '6px', verticalAlign: 'middle' }} />
                      Dúvidas
                    </button>
                  </div>
                </div>
